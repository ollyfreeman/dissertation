...
numbersep=5mm, numbers=left,numberstyle=\footnotesize,keywordstyle=\color{blue}\sf,identifierstyle=\color{magenta},showstringspaces=false}
...
\noindent{\bfseries Valid location}\\
...
\noindent{\bfseries Line of sight}\\
...
\noindent{\bfseries Path through a map}\\
...
\noindent{\bfseries Visibility graph: $d_{V}(L_{O},M) \rightarrow G_{v}(M)$}\\
\noindent
...
\noindent
The pseudocode for {\em Lazy Theta*} differs from that of {\em Theta*} in the {\tt Expand} subroutine, and also performs an {\tt Initialise} step on each node at the start of each expansion.
...
\noindent
Each block $B_{i,j}$ contains nodes that are collocational with nodes from up to eight neighbouring blocks. Figure 2.6 shows collocational nodes as two or four partially overlapping red circles. {\em Block A*} maintains the invariant for two collocational nodes $n$ and $n'$ that $n.g = n'.g$ and $n.h = n'.h$ --- the enforcement of this invariant enables shortest path information to flow between adjacent blocks.\\
...
\noindent{\bfseries Initialisation}\\
\noindent
...
\begin{figure}
   ...
   %angle check
    ...
    \node[above] at (20.5,12) {(d)};
    
    \draw[->] (23,9.5) -- (26,9.5);
    \draw (24.5,15) -- (27,15);
    \draw (24.5,-12) -- (27,-12);
    \draw (24.5,-12) -- (24.5,15);
    \node at (26,1.5) {$LDDB_{3}$};
    ...
    \fill[red] (-7,-5) circle (5pt);
    \node[above right, red] at (-7,-4) {$egress$};
    \node[below] at (-8.5,-8) {(h)};
    
    \node[below right,align=left] at (-10,-11) {(a) the original query requests the intermediate coordinates on the shortest path between \\ an $ingress$ of $(2,0)$ and an $egress$ of $(3,3)$ on the map shown, i.e. a map of size $3^{2}$ \\ with integer code 48.};
    
    \node[below right,align=left] at (-10,-15) {(b) the integer codes are calculated for all four rotations of the map, and the smallest code \\ is used for the query. The $ingress$ and $egress$ are rotated accordingly.};
    
    \node[below right,align=left] at (-10,-18) {(c) the $y-value$ of the rotated $ingress$ coordinate is larger than that of the rotated $egress$ \\ coordinate. Therefore, the $ingress$ and $egress$ are exchanged (i.e. reflected) for the query.};
    
    \node[below right,align=left] at (-10,-21) {(d) the final configuration for the query has been reached: map code 18 and \\ $ingress-egress$ coordinates $(3,0)-(0,1)$ which is encoded as $011-000-000-001$.};
    
    \node[below right,align=left] at (-10,-24) {(e) the query returns a 64 bit integer, whose 32 most significant bits can be decoded as a {\tt float} \\ giving the length of the shortest path between $ingress$ and $egress$, and whose 24 least significant \\ bits are $010-010-001-010$ which is decoded as $i_{1}=(2,2)$ and $i_{2}=(1,2)$.};
    
    \node[below right,align=left] at (-10,-28) {(f) since the coordinates were reflected in step (c), the reflection is undone in step (f). Therefore \\ the first intermediate coordinate $i_{1}=(1,2)$ and the second $i_{2}=(2,2)$.};
    
    \node[below right,align=left] at (-10,-31) {(g) since the map was rotated 90$^{\circ}$ clockwise in step (b), the rotation is undone in step (g). \\ Therefore $i_{1}=(1,1)$ and $i_{2}=(1,2)$.};
    
    \node[below right,align=left] at (-10,-34) {(h) the result is ready to be returned, which provides {\em Block A*} with the shortest path between \\ the $ingress$ and the $egress$, as shown.};
...
\noindent
Having carefully set up the problem in the {\bfseries Preparation} chapter, the core part of the implementation of the algorithm is a transparent translation of the pseudo-code in {\bfseries Algorithm 6} using similar design choices to the other pathfinding algorithms. However, the pseudo-code does not specify the implementation of the creation of {\tt Init}, {\tt TraceBack} or the creation of blocks, which also required the application of many of many of the techniques and design choices seen in the other algorithms, but to a far more complicated level. The {\tt BlockAStar} class itself is over 400 lines of Java (excluding comments), which doesn't include the implementation of blocks or the creation and decoding of the various $LDDB$ implementations. A selection of these classes can be found in {\bfseries Appendix ?} for the more strong-stomached readers.\\
...
\noindent{\em Core structural classes} --- basic functionality is unit tested. See Figure 3.13.\\
...
\centering
  ...
  \begin{tikzpicture}
    ...
    \node at (0,0.7) {\includegraphics[width=.9\textwidth]{strange.png}};
  \end{tikzpicture}
...
\centering
  ...
  \begin{tikzpicture}
  \filldraw[color=white,fill=white] (-3,-4.5) rectangle (3,4.5); 
  \node at (0,1) {\includegraphics{oscillation.png}};
  \end{tikzpicture}
...
\centering
  ...
  \begin{tikzpicture}
    \filldraw[color=white,fill=white] (-3,-4.5) rectangle (3,4.5); 
    \node at (0,0.7) {\includegraphics[width=.9\textwidth]{nodesExpanded.png}};
  \end{tikzpicture}
...
\node at (0,0) {
    ...
    \hline
    {\bfseries Block size 2 (units in KB)}         & Standard & Semi-extended & Fully-extended \\ \hline
    Uncompressed                        & 27       & 31            & 36             \\ \hline
    Bitwise-compressed                  & 11       & 13            & 15             \\ \hline
    Geometrically-compressed            & 6        & 6             & 7              \\ \hline
    Bitwise \& geometrically-compressed & 3        & 3             & 3              \\ \hline
    \end{tabular}
    };
...
\node at (0,-4) {
    \begin{tabular}{| l | l | l | l |}
    \hline
    {\bfseries Block size 3 (units in KB)}          & Standard & Semi-extended & Fully-extended \\ \hline
    Uncompressed                        & 1788     & 2506          & 3564           \\ \hline
    Bitwise-compressed                  & 726      & 1057          & 1520           \\ \hline
    Geometrically-compressed            & 248      & 358           & 468            \\ \hline
    Bitwise \& geometrically-compressed & 104      & 154           & 203            \\ \hline
    \end{tabular}
    };
...
\node at (0,-8) {
    \begin{tabular}{| l | l | l | l |}
    \hline
    {\bfseries Block size 4 (units in MB)}         & Standard & Semi-extended & Fully-extended \\ \hline
    Uncompressed                        & 401     & 677          & ?           \\ \hline
    Bitwise-compressed                  & 156      & 275          & 482           \\ \hline
    Geometrically-compressed            & 51      & 90           & 143            \\ \hline
    Bitwise \& geometrically-compressed & 20      & 37           & 59            \\ \hline
    \end{tabular}
    };
    \end{tikzpicture}
    
\caption{Size on disk of different $LDDB$ implementations}
\end{figure}
...
\node at (0,0) {
    ...
    \hline
    {\bfseries Execution time (s)}   & \multicolumn{3}{l |}{Block size} \\ \hline
    ~                                   & 2          & 3     & 4     \\ \hline
    Uncompressed                        & 10.53      & 9.53  & 15.79 \\ \hline
    Bitwise-compressed                  & 11.05      & 9.78  & 8.97  \\ \hline
    Geometrically-compressed            & 12.25      & 10.75 & 10.28 \\ \hline
    Bitwise \& geometrically-compressed & 14.38      & 11.91 & 11.44 \\ \hline
    \end{tabular}
    };
\end{tikzpicture}
...
\node at (0,0) {
    \begin{tabular}{| l | p{1.5cm} | p{1.5cm} | p{1.5cm} | }
    \hline
    {\bfseries Load times (s)}    & \multicolumn{3}{l |}{Block size} \\ \hline
    ~                                   & 2          & 3     & 4     \\ \hline
    Uncompressed                        & 0.130      & 1.335  & 334.071 \\ \hline
    Bitwise-compressed                  & 0.012 &  0.390 & 115.863 \\ \hline
    Geometrically-compressed            & 0.011 & 0.174 & 42.510  \\ \hline
    Bitwise \& geometrically-compressed & 0.005 & 0.061 &  13.161  \\ \hline
    \end{tabular}
    };
...
\node at (0,-4) {
    \begin{tabular}{| l | p{1.5cm} | p{1.5cm} | p{1.5cm} | }
    \hline
    {\bfseries Maximum execution time (ms)}  & \multicolumn{3}{l |}{Block size} \\ \hline
    ~                                   & 2                        & 3     & 4     \\ \hline
    Uncompressed                        & 117.78      & 98.6  & 108.4 \\ \hline
    Bitwise-compressed                  & 105.6 &  93.4 & 422.4 \\ \hline
    Geometrically-compressed            & 168.9 &115.5 & 682.0  \\ \hline
    Bitwise \& geometrically-compressed & 192.6 & 100.8 &  93.4  \\ \hline
    \end{tabular}
    };
    
    \end{tikzpicture}
\caption{Tradeoffs between different $LDDB$ implementations}
\end{figure}

\noindent
For these reasons, a compromise of the {\em bitwise-compressed} implementation of block size 3 is used for the remainder of this project due to its consistency and low memory footprint\footnote{To avoid frequent, costly garbage collection while creating graph structures for block size of 2 and to avoid heap overflows while loading $LDDB$s of block size 4, the tests were run with 2GB of heap space. The bitwise-compressed implementation of block size 3 does not have such excessive memory requirements}.
