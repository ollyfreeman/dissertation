...
public class Block_geobit extends Block{
	
	private int rotation;
	private Coordinate[][][] rotationMatrix = initialiseRotationMatrix();
	
	public Block_geobit(Map map, Coordinate topLeft, Coordinate goal) {
		super(map, topLeft, goal);
		code = getCodeAndRotation(map,size,topLeft).get0();
		rotation = getCodeAndRotation(map,size,topLeft).get1();
	}
