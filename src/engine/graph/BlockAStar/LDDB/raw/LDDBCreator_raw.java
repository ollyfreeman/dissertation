...
/*
 * Raw implementation of the LDDB, where I use an ArrayList of HashMaps of unencoded types (except PairOfCoords):
 * PairOfCoords : i.e. where the PairOfCoords in an encoded byte
 * Double
 * ArrayList<Coordinate>
 */

public class LDDBCreator_raw {

	private static int blockSize = 4;
	
	public static void createDB() {
		int totalMaps = (int) Math.pow(2,blockSize*blockSize);
		ArrayList<HashMap<PairOfCoords,Pair<Double,ArrayList<Coordinate>>>> db = new ArrayList<HashMap<PairOfCoords,Pair<Double,ArrayList<Coordinate>>>>();
		for(int mapCounter=0;mapCounter<totalMaps;mapCounter++) {
			...
			Map m = new Map(map);
			HashMap<PairOfCoords,Pair<Double,ArrayList<Coordinate>>> hm = new HashMap<PairOfCoords,Pair<Double,ArrayList<Coordinate>>>();
			if(mapCounter==8) {
				m.print();
			}
	...
	private static void f(HashMap<PairOfCoords,Pair<Double,ArrayList<Coordinate>>> hm, Map m, Coordinate sourceCoord, Coordinate goalCoord, int size, int mapCounter) {
		...
		if(n!=null) {
			...
			hm.put(new PairOfCoords(sourceCoord,goalCoord,blockSize),new Pair<Double,ArrayList<Coordinate>>(distanceAccumulator,intermediateCoordinates));
		}
	...
	private static void saveDB(ArrayList<HashMap<PairOfCoords,Pair<Double,ArrayList<Coordinate>>>> db) {
		String filename = "/Users/olly_freeman/Dropbox/Part2Project/"+blockSize+"zeroOLD.ser";
		try {
