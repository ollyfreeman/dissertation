package engine.graph.BlockAStar.LDDB.raw;
...
public class BlockAStarAlgorithm_raw {
	...
	private static ArrayList<HashMap<PairOfCoords,Pair<Double,ArrayList<Coordinate>>>> lddb = loadDB();
	...
	private static void expand(Block currentBlock, List<Coordinate> y) {
		for(Block neighbourBlock : currentBlock.getNeighbours()) {
			...
			for(int i=0;i<ListX.size();i++) {
				...
				for(Coordinate c : y) {
					double length = lddb.get(currentBlock.getCode()).get(new PairOfCoords(x,c,blockSize)) != null ? lddb.get(currentBlock.getCode()).get(new PairOfCoords(x,c,blockSize)).get0() : Double.POSITIVE_INFINITY;
	...
	private static Block initStart(Coordinate startCoord,Block[][] blockArray) {
        ...
        for(int i=0; i<blockSize;i++) {
			...
			for(Coordinate c : outArray) {
				...
				double length = lAIN.get0() != -1 ? lAIN.get0() : Double.POSITIVE_INFINITY;
	...
	private static Node postProcessing(Block startBlock, Block goalBlock) {
		...
		while(n != null && n.getParent() != null) {
			...
			if(n.getBlock() == parent.getBlock() && !n.getBlock().equals(startBlock)) {
				intermediateNodes = lddb.get(n.getBlock().getCode()).get(new PairOfCoords(new Coordinate(n.getX()-n.getBlock().getTopLeft().getX(),n.getY()-n.getBlock().getTopLeft().getY()),new Coordinate(parent.getX()-parent.getBlock().getTopLeft().getX(),parent.getY()-parent.getBlock().getTopLeft().getY()),blockSize)).get1();
	...
	private static ArrayList<HashMap<PairOfCoords,Pair<Double,ArrayList<Coordinate>>>> loadDB() {
		try {
			String filename = "/Users/olly_freeman/Dropbox/Part2Project/"+blockSize+"zero_raw"+".ser";
			FileInputStream fileIn = new FileInputStream(filename);
			ObjectInputStream in = new ObjectInputStream(fileIn);
			//System.out.print("Loading DB...");
			ArrayList<HashMap<PairOfCoords,Pair<Double,ArrayList<Coordinate>>>> lddb = (ArrayList<HashMap<PairOfCoords,Pair<Double,ArrayList<Coordinate>>>>) in.readObject();
