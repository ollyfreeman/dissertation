...
public class MapInstance_Static implements java.io.Serializable{
	...
	private AlgorithmData_Static aStarVisData;
	 ...
	 protected AlgorithmStatistics createAlgorithmData(AlgorithmType algorithmType, Coordinate source, Coordinate goal) {
		 ...
		 if(algorithmType.equals(AlgorithmType.AStarVisibility)) {
			 ...
			 if(aStarVisData == null) {
				 aStarVisData = new AlgorithmData_Static(AlgorithmType.AStarVisibility, graph, map);
			 }
			 ...
			 case Dijkstra:
				 if(dijkstraData == null) {
					 dijkstraData = new AlgorithmData_Static(AlgorithmType.Dijkstra, graph, map);
				 }
				 algorithmStatistics = new AlgorithmStatistics(dijkstraData);
				 break;
			 case AStar:
				 if(aStarData == null) {
					 aStarData = new AlgorithmData_Static(AlgorithmType.AStar, graph, map);
				 }
				 algorithmStatistics = new AlgorithmStatistics(aStarData);
				 break;
			 case AStarSmoothed: 
				 if(aStarSmoothedData == null) {
					 aStarSmoothedData = new AlgorithmData_Static(AlgorithmType.AStarSmoothed, graph, map);
				 }
				 algorithmStatistics = new AlgorithmStatistics(aStarSmoothedData);
				 break;
			 case ThetaStar:
				 if(thetaStarData == null) {
					 thetaStarData = new AlgorithmData_Static(AlgorithmType.ThetaStar, graph,map);
				 }
				 algorithmStatistics = new AlgorithmStatistics(thetaStarData);
				 break;
			 case LazyThetaStar:
				 if(lazyThetaStarData == null) {
					 lazyThetaStarData = new AlgorithmData_Static(AlgorithmType.LazyThetaStar, graph,map);
				 }
				 algorithmStatistics = new AlgorithmStatistics(lazyThetaStarData);
				 break;
			 case BlockAStar:
				 if(blockAStarData == null) {
					 blockAStarData = new AlgorithmData_Static(AlgorithmType.BlockAStar, graph,map);
				 }
